...
from .factories import ClientFactory, CommercialOpportunityFactory, ProjectFactory


@pytest.mark.django_db
def test_para_bucket_for_follows_link_type():
    client = ClientFactory()
    opportunity = CommercialOpportunityFactory(client=client)
...
def test_client_of_resolves_owner_from_any_link():
    ...
    assert drive.client_of(Document(commercial_opportunity=opportunity)) == client
