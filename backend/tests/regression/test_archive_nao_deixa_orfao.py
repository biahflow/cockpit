...
`account__archived_at`, arquivar um cliente sumia com ele da tela de Clientes e **mantinha** os
...
def test_cliente_com_oportunidade_aberta_nao_arquiva(admin_client: APIClient) -> None:
    ...
    resposta = admin_client.delete(reverse("client-detail", args=[opportunity.account.pk]))
...
def test_cliente_limpo_arquiva_e_leva_os_contatos_junto(admin_client: APIClient) -> None:
    ...
    contato = Contact.objects.create(account=cliente, first_name="Quem decide", email="a@b.test")
