"""Espelha a camada global da Engineering OS dentro deste repositório (ADR 0045).
...
Uso:
    ...
    uv run python ../.github/scripts/sync_engineering_os.py
    uv run python ../.github/scripts/sync_engineering_os.py --tag v0.2.0
    PULSE_EOS_ORIGIN=/caminho/para/fork uv run python scripts/sync_engineering_os.py
"""
...
# Um `.gitignore` aninhado mudaria a semântica de ignore do pulse dentro do diretório
...
def origin() -> str:
    return os.getenv("PULSE_EOS_ORIGIN") or DEFAULT_ORIGIN
...
por fora ([ADR 0045](../adr/0045-camada-global-vendorizada-e-pinada-por-tag.md)). Os arquivos
são cópia fiel da origem, em inglês, e **não são editados aqui** — nem este registro, que é
gerado pelo script.
...
Avançar o pino é trocar `PINNED_TAG` em `.github/scripts/sync_engineering_os.py` e rodar:

```bash
cd backend && uv run python ../.github/scripts/sync_engineering_os.py
```
