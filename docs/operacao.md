...
| Assinatura eletrônica | `ESIGN_ENABLED`, `ESIGN_PROVIDER` | fornecedor (Clicksign/DocuSign) | Esqueleto (desligado) |
...
- Assinatura: `ESIGN_ENABLED=true` + `ESIGN_PROVIDER=<clicksign|docusign>` — o adaptador do
  fornecedor precisa ser preenchido em `apps/core/esign.py` antes do uso real (esqueleto).
