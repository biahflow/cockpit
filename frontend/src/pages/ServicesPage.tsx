  ...
  return <section className="space-y-7">
    ...
    <section className="space-y-4">
      ...
      {levels.length ? <div className="grid gap-4 lg:grid-cols-3">{levels.map(service => <article className="grid gap-3 rounded-2xl border bg-white p-5 sm:p-6" key={service.id}>
        ...
        <button className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-ocean px-3 py-2.5 text-sm font-semibold text-white hover:bg-ink disabled:opacity-60" disabled={savingId === service.id} onClick={() => void save(service)}><Save className="size-4" />{savingId === service.id ? "Salvando…" : "Salvar"}</button>
    ...
    <section className="overflow-hidden rounded-2xl border bg-white">
      ...
      {loose.length ? <div className="divide-y">{loose.map(service => <div className="flex flex-wrap items-center gap-3 px-5 py-3 sm:px-6" key={service.id}>
        ...
        <button className="inline-flex items-center gap-1.5 rounded-xl bg-ocean px-3 py-2 text-sm font-semibold text-white hover:bg-ink disabled:opacity-60" disabled={savingId === service.id} onClick={() => void save(service)}><Save className="size-4" />{savingId === service.id ? "Salvando…" : "Salvar"}</button>
        ...
        <button className="grid size-9 place-items-center rounded-xl border text-slate-600 hover:bg-red-50 hover:text-signal" aria-label={`Arquivar serviço ${service.name}`} onClick={() => setArchiving(service)}><Trash2 className="size-4" /></button>
    ...
    <form className="flex flex-wrap items-end gap-3 rounded-2xl border bg-white p-5 sm:p-6" onSubmit={event => void create(event)}>
      ...
      <button className="inline-flex items-center gap-2 rounded-xl bg-ocean px-4 py-3 text-sm font-semibold text-white hover:bg-ink disabled:opacity-60" disabled={isCreating} type="submit"><Plus className="size-4" />{isCreating ? "Adicionando…" : "Adicionar serviço"}</button>
    </form>
  </section>;
}
