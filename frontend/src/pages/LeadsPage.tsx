import { ArrowRight, Building2, Inbox, Mail, MessageSquare, Phone, Sparkles, Trash2 } from "lucide-react";
...
const filters: (LeadStatus | "all")[] = ["all", "new", "contacted", "qualified", "discarded"];
...
export function LeadsPage() {
  ...
  return <section className="space-y-7">
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-ocean">Comercial</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink">Leads</h1><p className="mt-2 text-sm text-slate-500">Contatos recebidos pelo site. Triê e converta em oportunidades.</p></div><span className="self-start rounded-xl bg-mint px-3 py-2 text-sm font-semibold text-ocean sm:self-auto">{newCount} novos</span></header>
    {error && <p role="alert" className="rounded-xl bg-red-50 p-3 text-sm text-signal">{error}</p>}

    <div className="flex flex-wrap gap-2">{filters.map(value => <button key={value} className={`rounded-xl px-3 py-1.5 text-sm font-semibold transition ${filter === value ? "bg-ocean text-white" : "border bg-white text-slate-500 hover:text-ink"}`} onClick={() => setFilter(value)}>{value === "all" ? "Todos" : statusLabel[value]}</button>)}</div>
    ...
    {visible.length ? <div className="grid gap-4">{visible.map(lead => <article className="rounded-2xl border bg-white p-5 sm:p-6" key={lead.id}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0"><h2 className="text-base font-semibold text-ink">{lead.name}</h2><div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">{lead.company && <span className="flex items-center gap-1.5"><Building2 className="size-3.5" />{lead.company}</span>}<span className="flex items-center gap-1.5"><Mail className="size-3.5" />{lead.email}</span>{lead.phone && <span className="flex items-center gap-1.5"><Phone className="size-3.5" />{lead.phone}</span>}</div></div>
      ...
      {lead.ai_summary && <p className="mt-3 flex gap-2 rounded-xl bg-mint/50 p-3 text-sm text-ocean"><Sparkles className="mt-0.5 size-4 shrink-0" />{lead.ai_summary}{lead.ai_recommended_action && <span className="text-slate-500"> — {lead.ai_recommended_action}</span>}</p>}
      {lead.message && <p className="mt-3 flex gap-2 rounded-xl bg-slate-50 p-3 text-sm text-slate-600"><MessageSquare className="mt-0.5 size-4 shrink-0 text-slate-400" />{lead.message}</p>}
