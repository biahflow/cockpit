...
export function ProjectDetailPage({ id }: { id: number }) {
  ...
  return <section className="space-y-7">
    ...
    {editing && <form className="grid gap-4 rounded-2xl border bg-white p-5 sm:p-6" onSubmit={event => void saveProject(event)}>
      <div className="flex items-center justify-between"><h2 className="font-semibold text-ink">Editar projeto</h2><button type="button" className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-slate-100" aria-label="Cancelar edição" onClick={() => setEditing(false)}><X className="size-4" /></button></div>
    ...
    <section className="rounded-2xl border bg-white p-5 sm:p-6">
      ...
      {employees.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{employees.map(employee => <article className="rounded-xl border bg-slate-50/50 p-4" key={employee.id}>
        <div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold text-ink">{employee.name}</p><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${employee.status === "active" ? "bg-emerald-50 text-emerald-700" : employee.status === "paused" ? "bg-slate-100 text-slate-500" : "bg-amber-50 text-amber-700"}`}>{employee.status === "active" ? "Ativo" : employee.status === "paused" ? "Pausado" : "Em construção"}</span></div>
        {employee.area && <p className="mt-0.5 text-xs text-ocean">{employee.area}</p>}
        {employee.description && <p className="mt-1 text-xs text-slate-500">{employee.description}</p>}
      ...
      </article>)}</div> : <p className="mt-4 rounded-xl border border-dashed bg-slate-50/60 px-4 py-6 text-center text-sm text-slate-400">Nenhum funcionário digital ainda.</p>}
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <WorkColumn icon={<Flag className="size-4" />} title="Marcos" count={milestones.length}>
        <form className="grid gap-3 sm:grid-cols-[1fr_auto]" onSubmit={event => void createMilestone(event)}>
          ...
          <div className="flex gap-2"><input className="field" type="date" value={milestoneDraft.due_date} onChange={event => setMilestoneDraft({ ...milestoneDraft, due_date: event.target.value })} required /><button className="grid size-11 shrink-0 place-items-center rounded-xl bg-ocean text-white hover:bg-ink" aria-label="Adicionar marco" type="submit"><Plus className="size-4" /></button></div>
        </form>
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <WorkColumn icon={<Video className="size-4" />} title="Reuniões" count={meetings.length}>
        <form className="grid gap-3" onSubmit={event => void createMeeting(event)}>
          ...
          <div className="flex gap-2"><input className="field" type="date" value={meetingDraft.date} onChange={event => setMeetingDraft({ ...meetingDraft, date: event.target.value })} required /><input className="field" type="url" placeholder="Link da gravação (opcional)" value={meetingDraft.recording_url} onChange={event => setMeetingDraft({ ...meetingDraft, recording_url: event.target.value })} /><button className="grid size-11 shrink-0 place-items-center rounded-xl bg-ocean text-white hover:bg-ink" aria-label="Adicionar reunião" type="submit"><Plus className="size-4" /></button></div>
        ...
        {meetings.length ? <div className="divide-y">{meetings.map(meeting => <div className="py-3" key={meeting.id}>
          <div className="flex items-center gap-3">
            <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-500"><Video className="size-4" /></span>
            <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-ink">{meeting.title}</p><p className="mt-0.5 text-xs text-slate-500">{formatDate(meeting.date)}</p></div>
            {meeting.recording_url && <a className="shrink-0 rounded-lg p-1.5 text-slate-400 hover:text-ocean" href={meeting.recording_url} target="_blank" rel="noreferrer" aria-label={`Abrir gravação de ${meeting.title}`}><ExternalLink className="size-4" /></a>}
      ...
      <WorkColumn icon={<Inbox className="size-4" />} title="Pendências" count={pendencias.length}>
        <form className="grid gap-3" onSubmit={event => void createPendencia(event)}>
          ...
          <div className="flex gap-2"><select className="field" aria-label="Responsável pela pendência" value={pendenciaDraft.party} onChange={event => setPendenciaDraft({ ...pendenciaDraft, party: event.target.value as Party })}><option value="provider">Fornecedor</option><option value="client">Cliente</option></select><button className="grid size-11 shrink-0 place-items-center rounded-xl bg-ocean text-white hover:bg-ink" aria-label="Adicionar pendência" type="submit"><Plus className="size-4" /></button></div>
        </form>
        {pendencias.length ? <div className="divide-y">{pendencias.map(pendencia => {
          ...
          return <div className="flex items-center gap-3 py-3" key={pendencia.id}>
            ...
            <div className="min-w-0 flex-1"><p className={`truncate text-sm font-medium ${resolved ? "text-slate-400 line-through" : "text-ink"}`}>{pendencia.title}</p><p className="mt-0.5 text-xs text-slate-500">{partyLabel[pendencia.party]}</p></div>
        ...
        })}</div> : <p className="rounded-xl border border-dashed bg-slate-50/60 px-4 py-6 text-center text-sm text-slate-400">Nenhuma pendência.</p>}
      </WorkColumn>
    </div>
  </section>;
}
...
function AiScorePanel({ project, canManage, onTogglePublish }: { project: Project; canManage: boolean; onTogglePublish: (next: boolean) => void }) {
  ...
  return <section className="space-y-4 rounded-2xl border bg-white p-5 sm:p-6">
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex-1"><h2 className="font-semibold text-ink">AI Score</h2><p className="text-sm text-slate-500">Maturidade e oportunidade de IA a partir do Discovery/Assessment.</p></div>
...
function JourneySection({ phases, canManage, onAdvance, onMark, onSetTarget }: { phases: ProjectPhase[]; canManage: boolean; onAdvance: () => void; onMark: (id: number) => void; onSetTarget: (phaseId: number, date: string) => void }) {
  ...
  return <section className="space-y-5 rounded-2xl border bg-white p-5 sm:p-6">
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex-1"><h2 className="font-semibold text-ink">Jornada de Transformação</h2><p className="text-sm text-slate-500">{done} de {phases.length} fases concluídas</p></div>
    ...
    <div className="flex flex-wrap gap-2">{phases.map(phase => {
      ...
      return <span key={phase.id} className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold ${isDone ? "bg-emerald-50 text-emerald-700" : isActive ? "bg-ocean text-white" : "bg-slate-100 text-slate-400"}`}>{isDone ? <CheckCircle2 className="size-3.5" /> : isActive ? <MapPin className="size-3.5" /> : <Lock className="size-3.5" />}{phase.phase_name}</span>;
    })}</div>

    {active ? <div className="rounded-2xl border bg-slate-50/60 p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-wide text-ocean">Você está aqui</p><h3 className="mt-0.5 text-lg font-semibold text-ink">{active.phase_name}</h3>{active.phase_description && <p className="mt-1 text-sm text-slate-500">{active.phase_description}</p>}</div>
        {next && <p className="shrink-0 text-right text-xs text-slate-400">Próxima<span className="mt-0.5 flex items-center gap-1 font-semibold text-slate-600"><ChevronRight className="size-3.5" />{next.phase_name}</span></p>}
      </div>

      {active.deliverables.length > 0 && <div className="mt-4">
        <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Entregáveis · {active.deliverables.filter(item => item.status === "delivered").length}/{active.deliverables.length}</p>
        <div className="divide-y">{active.deliverables.map(item => {
          ...
          return <div className="flex items-center gap-3 py-2.5" key={item.id}>
            ...
            <span className={`flex-1 text-sm font-medium ${delivered ? "text-slate-400 line-through" : "text-ink"}`}>{item.name}</span>
          </div>;
        })}</div>
      </div>}

      <div className="mt-4 flex flex-wrap items-end justify-between gap-3">
        {canManage ? <label className="grid gap-1 text-xs font-medium text-slate-500">Previsão desta fase<input className="field w-44" type="date" value={active.target_date ?? ""} onChange={event => onSetTarget(active.id, event.target.value)} /></label> : active.target_date ? <p className="text-sm text-slate-500">Previsão: {formatDate(active.target_date)}</p> : <span />}
...
function WorkColumn({ icon, title, count, children }: { icon: ReactNode; title: string; count: number; children: ReactNode }) {
  return <section className="space-y-4 rounded-2xl border bg-white p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-mint text-ocean">{icon}</span><div><h2 className="font-semibold text-ink">{title}</h2><p className="text-sm text-slate-500">{count} {count === 1 ? "item" : "itens"}</p></div></div>{children}</section>;
}
...
function WorkList({ items, onComplete, onCalendar, emptyLabel }: { items: (Milestone | Task)[]; onComplete: (id: number) => void; onCalendar?: (id: number) => void; emptyLabel: string }) {
  if (!items.length) return <p className="rounded-xl border border-dashed bg-slate-50/60 px-4 py-6 text-center text-sm text-slate-400">{emptyLabel}</p>;
  return <div className="divide-y">{items.map(item => {
    ...
    return <div className="flex items-center gap-3 py-3" key={item.id}>
      ...
      <div className="min-w-0 flex-1"><p className={`truncate text-sm font-medium ${done ? "text-slate-400 line-through" : "text-ink"}`}>{item.title}</p><p className={`mt-0.5 flex items-center gap-1.5 text-xs ${item.is_overdue ? "font-semibold text-signal" : "text-slate-500"}`}>{item.is_overdue && <AlertTriangle className="size-3.5" />}{formatDate(item.due_date)}</p></div>
      {onCalendar && <button className="shrink-0 rounded-lg p-1.5 text-slate-400 hover:text-ocean" aria-label={`Adicionar ${item.title} ao calendário`} onClick={() => onCalendar(item.id)}><CalendarPlus className="size-4" /></button>}
