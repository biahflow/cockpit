...
export function DocumentsPage() {
  ...
  return <section className="space-y-7">
    ...
    />}
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-accent">Documentos</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink">Arquivos do portal</h1><p className="mt-2 text-sm text-slate-600">Documentos privados vinculados a clientes, oportunidades ou projetos.</p></div><span className="self-start rounded-xl bg-accent-50 px-3 py-2 text-sm font-semibold text-accent sm:self-auto">{documents.length} arquivos</span></header>
    ...
    <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      <form className="space-y-4 rounded-2xl border bg-white p-5 sm:p-6" onSubmit={event => void upload(event)}>
        <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-accent-50 text-accent"><UploadCloud className="size-5" /></span><div><h2 className="font-semibold text-ink">Enviar documento</h2><p className="text-sm text-slate-600">Vincule a exatamente um recurso.</p></div></div>
        ...
        <label className="grid gap-2 text-sm font-medium text-slate-700">Arquivo<input className="field file:mr-3 file:rounded-lg file:border-0 file:bg-accent-50 file:px-3 file:py-1 file:text-accent" type="file" ref={fileInput} /></label>
        <button className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-ink px-4 py-3 text-sm font-semibold text-white hover:bg-ink disabled:opacity-60" type="submit" disabled={isUploading}><UploadCloud className="size-4" />{isUploading ? "Enviando…" : "Enviar documento"}</button>
      </form>
