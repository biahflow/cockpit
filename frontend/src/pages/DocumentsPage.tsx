  ...
  return <section className="space-y-7">
    ...
    <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      <form className="panel space-y-4 sm:p-6" onSubmit={event => void upload(event)}>
        ...
        <label className="form-label">Vincular a<select className="field" value={linkType} onChange={event => { setLinkType(event.target.value as LinkType); setTarget(""); }}>{!isDelivery && <option value="client">Cliente</option>}{!isDelivery && <option value="opportunity">Oportunidade</option>}<option value="project">Projeto</option></select></label>
