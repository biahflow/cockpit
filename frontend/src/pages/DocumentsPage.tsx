...
import { finalidadesPara, rotuloDaFinalidade } from "../documents";
...
export function DocumentsPage() {
  const { esignEnabled, esignHouseSignerEmail, user } = useAuth();
  ...
  const [signing, setSigning] = useState<DocumentEntry | null>(null);
  ...
  return <section className="space-y-7">
    ...
    />}
    {signing && <RequestSignatureModal
      document={signing} houseEmail={esignHouseSignerEmail}
      onClose={() => setSigning(null)} onSent={load}
    />}
    ...
    <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      <form className="panel space-y-4 sm:p-6" onSubmit={event => void upload(event)}>
        ...
        <label className="form-label">Vincular a<select className="field" value={linkType} onChange={event => {
          const novo = event.target.value as LinkType;
          setLinkType(novo); setTarget("");
        ...
        {/* Sempre visível, com as opções filtradas pelo vínculo (DAP r2, A1-r2). O que exige conta
            ...
            conta do acordo com a do mandato. Amarrando a condição às opções, o NDA passa a poder
            ser classificado onde ele de fato está, e a regra da casa continua valendo: a tela não
            oferece o que a API recusaria. */}
        <label className="form-label">Finalidade<select className="field" value={kind} onChange={event => setKind(event.target.value)}><option value="">Documento comum</option>{finalidadesPara(linkType).map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
