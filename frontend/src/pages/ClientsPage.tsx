...
export function ClientsPage() {
  ...
  return <section className="space-y-7"><header className="page-head flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="eyebrow">Relacionamento</p><h1>Clientes</h1><p>Mantenha a base que sustenta seus projetos.</p></div><span className="rounded-xl bg-brand-50 px-3 py-2 text-sm font-semibold text-brand-700">{filter === "archived" ? `${archived.length} arquivados` : `${clients.length} cadastrados`}</span></header>{error && <p role="alert" className="alert--error">{error}</p>}
    <div className="grid gap-5 lg:grid-cols-[.72fr_1.28fr]"><form className="panel sm:p-6" onSubmit={event => void submit(event)}><div className="flex items-center gap-3"><span className="metric-icon size-10"><Plus className="size-5" /></span><div><h2 className="font-semibold text-ink">Novo cliente</h2><p className="text-sm text-slate-600">Comece pelo nome principal.</p></div></div><label className="mt-6 form-label">Nome do cliente<input className="field" value={name} onChange={event => setName(event.target.value)} placeholder="Ex.: Empresa Exemplo" required /></label><label className="mt-4 form-label">Razão social<input className="field" value={legalName} onChange={event => setLegalName(event.target.value)} placeholder="Opcional" /></label><label className="mt-4 form-label">CNPJ / CPF<input className="field" value={taxId} onChange={event => setTaxId(event.target.value)} placeholder="Opcional" /></label><label className="mt-4 form-label">Situação<select className="field" value={status} onChange={event => setStatus(event.target.value as ClientStatus)}><option value="prospect">Prospect — ainda não fechou</option><option value="active">Cliente ativo — já fechou</option></select></label><button className="btn mt-4 w-full" disabled={isCreating} type="submit"><Plus className="size-4" />{isCreating ? "Cadastrando…" : "Cadastrar cliente"}</button></form>
