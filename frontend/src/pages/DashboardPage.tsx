...
import { FdeLadderOverview } from "../components/FdeLadder";
...
export function DashboardPage() {
  ...
  return <section className="space-y-7"><header className="page-head flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="eyebrow">Visão geral</p><h1 className="sm:text-4xl">Sua operação, em movimento.</h1><p>Acompanhe a saúde comercial e as entregas que pedem atenção.</p></div><a href="/comercial" className="btn self-start sm:self-auto">Ver pipeline <ArrowUpRight className="size-4" /></a></header>
    ...
    {/* A escada FDE por conta (FDD 042) logo abaixo dos cartões: é a varredura da carteira — onde
        cada conta está, de quem é a bola e há quanto tempo parou. Ordem, teto de linhas e o limiar
        de "parado" chegam prontos do backend; recalcular qualquer um aqui seria a segunda
        definição da regra. Some quando não há conta em degrau aberto. */}
    <FdeLadderOverview rows={data.account_ladder} />
