...
export function SettingsPage() {
  ...
  return <section className="space-y-7">
    <header><p className="text-sm font-semibold text-ocean">Administração</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink">Configurações</h1><p className="mt-2 text-sm text-slate-500">Ligue ou desligue integrações sem novo deploy. Credenciais continuam no ambiente.</p></header>
    ...
    <section className="overflow-hidden rounded-2xl border bg-white">
      ...
      {flags.length ? <div className="divide-y">{flags.map(flag => <div className="flex flex-wrap items-center gap-4 px-5 py-4 sm:px-6" key={flag.key}>
        <div className="min-w-0 flex-1"><p className="text-sm font-semibold text-ink">{flag.label}</p><p className="mt-0.5 text-xs text-slate-500">{hint(flag)}</p></div>
        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${flag.enabled ? "bg-mint text-ocean" : "bg-slate-100 text-slate-500"}`}>{flag.enabled ? "Ligada" : "Desligada"}</span>
        ...
        {flag.toggleable
          ...
          : <span className="text-xs font-medium text-slate-400">via .env</span>}
      </div>)}</div> : <p className="px-6 py-6 text-center text-sm text-slate-400">Carregando integrações…</p>}
    </section>
  </section>;
}
