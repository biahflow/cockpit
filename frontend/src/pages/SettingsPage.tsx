...
export function SettingsPage() {
  ...
  return <section className="space-y-7">
    <header><p className="text-sm font-semibold text-accent">Administração</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink">Configurações</h1><p className="mt-2 text-sm text-slate-600">Ligue ou desligue integrações sem novo deploy. Credenciais continuam no ambiente.</p></header>
    {error && <p role="alert" className="rounded-xl bg-red-50 p-3 text-sm text-danger">{error}</p>}
    {syncResult && <p role="status" className="rounded-xl bg-accent-50/60 p-3 text-sm text-accent">{syncResult}</p>}

    <section className="overflow-hidden rounded-2xl border bg-white">
      <div className="flex items-center gap-3 border-b px-5 py-4 sm:px-6"><span className="grid size-9 place-items-center rounded-xl bg-accent-50 text-accent"><Settings className="size-4" /></span><div><h2 className="font-semibold text-ink">Integrações</h2><p className="mt-0.5 text-sm text-slate-600">Estado atual de cada recurso conectável.</p></div></div>
      {flags.length ? <div className="divide-y">{flags.map(flag => <div className="flex flex-wrap items-center gap-4 px-5 py-4 sm:px-6" key={flag.key}>
        ...
        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${flag.enabled ? "bg-accent-50 text-accent" : "bg-slate-100 text-slate-600"}`}>{flag.enabled ? "Ligada" : "Desligada"}</span>
