    ...
    <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      ...
      <div className="space-y-5">
        <section className="panel panel--flush">
          ...
          {invitations.length ? <div className="panel-rows">{invitations.map(invitation => { const status = invitationStatus(invitation); return <div className="row gap-3 py-3" key={invitation.id}><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-slate-100 text-muted"><MailPlus className="size-4" /></span><div className="row-main"><strong className="truncate">{invitation.email}</strong><span>{roleLabel[invitation.role]}</span></div><div className="row-meta"><span className={`state ${status.cls}`}>{status.label}</span></div></div>; })}</div> : <p className="px-6 py-6 text-center text-sm text-muted">Nenhum convite ainda.</p>}
        </section>
        <section className="panel panel--flush">
          <div className="panel-heading"><h2>Usuários ativos</h2></div>
          {users.length ? <div className="panel-rows">{users.map(user => <div className="row gap-3 py-3" key={user.id}><span className="metric-icon">{user.is_admin ? <ShieldCheck className="size-4" /> : <UserRound className="size-4" />}</span><div className="row-main"><strong className="truncate">{user.first_name || user.username}</strong><span className="truncate">{user.email || user.username}</span></div><div className="row-meta"><span className="state state--off">{user.is_admin ? roleLabel.admin : roleLabel[user.role]}</span></div></div>)}</div> : <p className="px-6 py-6 text-center text-sm text-muted">Nenhum usuário.</p>}
        </section>
      </div>
    </div>
  </section>;
