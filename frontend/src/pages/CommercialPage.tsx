...
import type { Client, Contact, DocumentEntry, Opportunity, PipelineStage } from "../types";

const money = new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", maximumFractionDigits: 0 });
const blankDraft = { title: "", client: "", estimated_value: "", stage: "", expected_close_date: "" };
...
export function CommercialPage() {
  ...
  const [detailDraft, setDetailDraft] = useState({ title: "", scope: "", estimated_value: "", expected_close_date: "", contact: "", stage: "" });
  ...
  const load = useCallback(() => Promise.all([api<PipelineStage[]>("/pipeline-stages/"), api<Opportunity[]>("/opportunities/"), api<Client[]>("/clients/")]).then(([loadedStages, loadedOpportunities, loadedClients]) => { setStages(loadedStages); setOpportunities(loadedOpportunities); setClients(loadedClients); if (loadedStages[0]) setDraft(current => current.stage ? current : { ...current, stage: String(loadedStages[0].id) }); }).catch((cause: Error) => setError(cause.message)), []);
  ...
  async function createOpportunity(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { await api("/opportunities/", { method: "POST", body: JSON.stringify({ ...draft, client: Number(draft.client), stage: Number(draft.stage) }) }); setDraft(current => ({ ...blankDraft, stage: current.stage })); setComposerOpen(false); await load(); } catch (cause) { setError((cause as Error).message); } }
  ...
  async function openDetail(item: Opportunity) {
    setDetail(item);
    setDetailDraft({ title: item.title, scope: item.scope, estimated_value: item.estimated_value, expected_close_date: item.expected_close_date, contact: item.contact ? String(item.contact) : "", stage: String(item.stage) });
  ...
  return <section className="space-y-7"><header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-ocean">Comercial</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink">Pipeline de oportunidades</h1><p className="mt-2 text-sm text-slate-500">Mova negociações adiante e transforme ganhos em projetos.</p></div><div className="flex flex-wrap items-center gap-3 self-start sm:self-auto">{user?.role === "admin" && <a href="/pipeline" className="inline-flex items-center gap-2 rounded-xl border bg-white px-4 py-2.5 text-sm font-semibold text-ink hover:border-ocean"><SlidersHorizontal className="size-4 text-ocean" />Configurar etapas</a>}<button className="inline-flex items-center gap-2 rounded-xl bg-ocean px-4 py-2.5 text-sm font-semibold text-white hover:bg-ink" onClick={() => setComposerOpen(true)}><Plus className="size-4" />Nova oportunidade</button></div></header>{error && <p role="alert" className="rounded-xl bg-red-50 p-3 text-sm text-signal">{error}</p>}
    ...
    {detail && <Modal title="Detalhe da oportunidade" onClose={() => setDetail(null)}>
      ...
      <div className="mt-6 border-t pt-5">
        ...
        {detailDocs.length ? <div className="mt-3 divide-y">{detailDocs.map(doc => <div className="flex items-center gap-3 py-2" key={doc.id}><FileText className="size-4 shrink-0 text-slate-400" /><span className="min-w-0 flex-1 truncate text-sm text-ink">{doc.original_name}</span>{doc.drive_link && <a className="shrink-0 rounded-lg px-2 py-1 text-xs font-semibold text-slate-400 hover:text-ocean" href={doc.drive_link} target="_blank" rel="noreferrer" aria-label={`Abrir ${doc.original_name} no Drive`}>Drive</a>}<a className="shrink-0 rounded-lg p-1.5 text-slate-400 hover:text-ocean" href={documentDownloadUrl(doc.id)} aria-label={`Baixar ${doc.original_name}`}><Download className="size-4" /></a></div>)}</div> : <p className="mt-3 text-sm text-slate-400">Nenhum documento vinculado.</p>}
