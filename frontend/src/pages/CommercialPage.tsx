  ...
  return <section className="space-y-7"><header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-accent">Comercial</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink">Pipeline de oportunidades</h1><p className="mt-2 text-sm text-slate-600">Mova negociações adiante e transforme ganhos em projetos.</p></div><div className="flex flex-wrap items-center gap-3 self-start sm:self-auto">{user?.is_admin && <a href="/pipeline" className="inline-flex items-center gap-2 rounded-xl border bg-white px-4 py-2.5 text-sm font-semibold text-ink hover:border-accent"><SlidersHorizontal className="size-4 text-accent" />Configurar etapas</a>}<div className="flex gap-1 rounded-xl bg-slate-100 p-1"><button className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${showArchived ? "text-slate-600 hover:text-ink" : "bg-white text-accent shadow-sm"}`} onClick={() => setShowArchived(false)}>Pipeline</button><button className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${showArchived ? "bg-white text-accent shadow-sm" : "text-slate-600 hover:text-ink"}`} onClick={() => setShowArchived(true)}>Arquivadas</button></div><button className="inline-flex items-center gap-2 rounded-xl bg-ink px-4 py-2.5 text-sm font-semibold text-white hover:bg-ink" onClick={() => setComposerOpen(true)}><Plus className="size-4" />Nova oportunidade</button></div></header>{error && <p role="alert" className="rounded-xl bg-red-50 p-3 text-sm text-danger">{error}</p>}
    ...
    {showArchived ? <section className="overflow-hidden rounded-2xl border bg-white">
      ...
      {archived.length ? <div className="divide-y">{archived.map(item => <div className="flex flex-wrap items-center gap-3 px-5 py-4 sm:px-6" key={item.id}>
        ...
        <button className="inline-flex shrink-0 items-center gap-1.5 rounded-xl border px-3 py-2 text-sm font-semibold text-accent hover:border-accent disabled:opacity-60" disabled={restoring === item.id} onClick={() => void restore(item.id)}><RotateCcw className="size-4" />{restoring === item.id ? "Restaurando…" : "Restaurar"}</button>
    ...
    {detail && <Modal title="Detalhe da oportunidade" width="3xl" onClose={() => setDetail(null)}>
      <form className="grid gap-4" onSubmit={event => void saveDetail(event)}>
        ...
        <div className="flex flex-wrap gap-3"><button className="inline-flex items-center gap-2 rounded-xl bg-ink px-4 py-3 text-sm font-semibold text-white hover:bg-ink" type="submit"><Save className="size-4" />Salvar</button>{stages.find(stage => stage.id === detail.stage)?.kind === "won" && (detail.project_archived
          ...
          : detail.project
          ? <a className="inline-flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold text-ink hover:border-accent" href={`/projetos/${detail.project}`}>Ver projeto <ArrowRight className="size-4 text-emerald-600" /></a>
          : <button type="button" className="inline-flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold text-ink hover:border-accent" onClick={() => startConversion(detail)}>Converter em projeto <ArrowRight className="size-4 text-accent" /></button>)}
          {user?.is_admin && <button type="button" className="ml-auto inline-flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold text-slate-600 hover:border-danger hover:text-danger" onClick={() => setArchiving(detail)}><Trash2 className="size-4" />Arquivar</button>}</div>
      </form>
...
const tierBadges: Record<ServiceTier, string> = {
  discovery_express: "bg-accent-50 text-accent",
  discovery_assessment: "bg-accent-50 text-accent-700",
