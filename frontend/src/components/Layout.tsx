import { BarChart3, Bell, BookMarked, BookOpen, ChevronDown, FileText, FolderKanban, HandCoins, Inbox, LayoutDashboard, LogOut, Menu, Package, Receipt, Route, Settings, Trophy, UserRound, Users, UsersRound, WalletCards } from "lucide-react";
...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-canvas lg:grid lg:grid-cols-[254px_1fr]">
    <aside className="sidebar hidden lg:flex">
      {/* O produto se apresenta como **Pulse**; Biahflow é a casa, e fica no subtítulo (ADR 0043,
          DAP GH-26 r1). O lockup é o componente e o mark é o asset canônico — colar o SVG inline
          aqui é o que `assets/brand/README.md` proíbe, porque duplica a geometria da marca. */}
      <a href="/" className="brand-row"><PulseBrand /></a>
    ...
    <div className="min-w-0">
      <header className="topbar">
        ...
        <p className="breadcrumb">Pulse{currentLabel && <><b>/</b><strong>{currentLabel}</strong></>}</p>
        <div className="topbar-actions">
          ...
          <div className="relative"><button className="user-button" aria-haspopup="menu" aria-expanded={isUserMenuOpen} ref={botaoMenu} onClick={() => setUserMenuOpen(open => !open)}><Avatar user={user} name={displayName} /><span className="hidden text-left sm:block"><span className="block text-xs font-semibold text-ink">{displayName}</span><span className="block text-[11px] text-muted">{labelFor(user)}</span></span><ChevronDown className={`size-3 text-muted transition ${isUserMenuOpen ? "rotate-180" : ""}`} /></button>{isUserMenuOpen && <><button className="fixed inset-0 z-30 cursor-default" aria-label="Fechar menu" onClick={() => setUserMenuOpen(false)} /><div className="absolute right-0 top-full z-40 popover popover-menu mt-2 w-56" role="menu"><div className="popover-user"><strong>{displayName}</strong><small>{user.email || labelFor(user)}</small></div>{/* Neutro como o "Sair", e não com realce clay: o destaque do board serve para localizar a
              mudança ali, não para pintar o item na tela. É `<a>` e não `<button>` porque navega —
              o produto roteia por `pathname`, e um botão daria a um link o papel errado. */}<a className="popover-item" role="menuitem" href="/perfil"><UserRound className="size-4" />Meu perfil</a><button className="popover-item hover:text-danger" role="menuitem" onClick={() => void handleLogout()}><LogOut className="size-4" />Sair</button></div></>}</div>
        </div>
      </header>
      {/* A gaveta ganha o lockup denso — mark + wordmark, sem subtítulo: no celular a barra lateral
          não está em cena e sem ele o menu abre sem dizer de que produto é. **Preservada e não
          melhorada**: continua sem overlay, sem `focus trap`, sem `aria-modal` e sem `Escape` —
          nada disso existe hoje, e acrescentá-los está explicitamente reservado no DAP GH-26 r1.
          O menu continua sendo o mesmo `nav()` do sidebar, com o mesmo filtro por papel. */}
      {isMobileMenuOpen && <div className="border-b border-line bg-white p-4 lg:hidden"><a href="/" className="brand-row"><PulseBrand subtitle={null} /></a>{nav()}</div>}
